...
**Data layer:** All project content lives in `src/lib/projects.ts` as a typed `Project[]` array. The `id` field is the URL slug. `featured: true` controls which project appears in the home page hero. To add/edit projects, only this file needs to change. Key typed fields: `type` must be one of `'Residence' | 'Development' | 'Condominium' | 'Commercial'`; `status` must be one of `'Built' | 'Under Construction' | 'Design Development' | 'Concept'`.
...
**Typography:** Cormorant Garamond (Google Fonts), weight 300 throughout, italic for expressive moments. Loaded via `next/font/google` in `src/app/layout.tsx` as CSS variable `--font-cormorant`. Note: `globals.css` also contains a legacy `@import` for the same font — the `next/font` load takes precedence.
